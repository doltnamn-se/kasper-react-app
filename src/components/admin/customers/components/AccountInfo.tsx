...
export const AccountInfo = ({ customer, isOnline = false, userLastSeen = null, onCopy }: AccountInfoProps) => {
  const { t } = useLanguage();
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-1">
        <p className="text-xs font-bold text-[#000000] dark:text-[#FFFFFF]">{t('customer.type')}</p>
        <span className="text-xs font-medium text-[#000000] dark:text-[#FFFFFF]">
      ...
      {!isOnline && userLastSeen && (
        <div className="space-y-1">
          ...
          <span className="text-xs font-medium text-[#000000] dark:text-[#FFFFFF]">
            {formatDateDistance(userLastSeen)}
          </span>
        </div>
      )}
    </div>
